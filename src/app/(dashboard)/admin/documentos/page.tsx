...
const tipoLabels: Record<string, { label: string; color: string; icon: any }> = {
  ...
  otro: { label: 'Otro', color: 'bg-slate-500/10 text-[var(--muted-foreground)]', icon: File },
};
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-black text-[var(--foreground)]">Gestión de Documentos</h2>
          <p className="text-[var(--muted-foreground)]">Administra boletos, vouchers y documentación de viajes</p>
        </div>
        <button 
      ...
      <div className="glass-card rounded-3xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {documentos.map((doc) => {
                ...
                return (
                  <tr key={doc.id} className="hover:bg-[var(--muted)] transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-[var(--foreground)]">{doc.nombre_archivo}</p>
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm text-[var(--foreground)] font-mono">{doc.venta?.codigo}</p>
                      <p className="text-xs text-[var(--muted-foreground)]">{doc.venta?.paquete_nombre}</p>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          ...
                          className="p-2 hover:bg-[var(--muted)] rounded-lg text-[var(--muted-foreground)] hover:text-blue-400 transition-all disabled:opacity-50"
                          title="Descargar"
                        >
          ...
          {!isLoading && documentos.length === 0 && (
            <div className="p-20 text-center text-[var(--muted-foreground)]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-lg rounded-3xl p-8">
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              <div>
                <label className="text-sm text-[var(--muted-foreground)] mb-1 block">Venta</label>
                <select
                  required
                  value={selectedVenta}
              ...
              <div>
                <label className="text-sm text-[var(--muted-foreground)] mb-1 block">Archivo</label>
                <input
                  type="file"
                  required
                  onChange={(e) => setUploadForm({...uploadForm, archivo: e.target.files?.[0] || null})}
                  className="w-full bg-[var(--muted)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--foreground)] outline-none focus:border-blue-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-[var(--foreground)] hover:file:bg-blue-700"
                />
                <p className="text-xs text-[var(--muted-foreground)] mt-1">Máximo 10MB. Formatos: PDF, JPG, PNG</p>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-[var(--muted)] hover:bg-white/10 text-[var(--foreground)] font-medium transition-all"
                >
                  Cancelar
                </button>
                <button
